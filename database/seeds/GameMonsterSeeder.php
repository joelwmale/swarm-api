...
class GameMonsterSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $attributes = GameAttribute::where('id', '<', 6)->get();

        foreach ($monsters as $monster) {
            $baseMonster = strlen($monster['id']) === 3;
            ...
            if ($baseMonster) {
                ...
                GameMonster::create(array_merge($monster, ['attribute_id' => GameAttribute::where('id', 6)->first()->id]));

                // Create each attribute
                foreach ($attributes as $attribute) {
                    GameMonster::create([
                        'attribute_id' => $attribute->id,
                        'id'      => ($monster['id']. 0 .$attribute->id),
                        'name'         => $monster['name'].' ('.$attribute->name.')',
                    ]);
                }
            } else {
                // Evolved monster already has the attribute type
                $attribute = GameAttribute::where('id', substr($monster['id'], -1))->first();
